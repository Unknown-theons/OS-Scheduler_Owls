...
def save_results_to_file(processes, filename='round_robin_results.txt'):
    with open(filename, 'w') as f:
        ...
        for proc in processes:
            f.write(f"{proc['id']:<12} {proc['arrival']:<13.2f} {proc['burst']:<11.2f} {proc['priority']:<10} "
                   f"{proc['completion']:<11.2f} {proc['turnaround']:<11.2f} {proc['waiting']:<8.2f}\n")
...
if __name__ == '__main__':
    ...
    # Print and save the scheduling results
    print_results(scheduled_processes)
    save_results_to_file(scheduled_processes, os.path.join(os.path.dirname(__file__),"..", "..", "round_robin_results.txt"))
